...
- **[Feb 2026] "Add to calendar" + ".ics download" links on EventCard**: exported `buildGoogleCalendarUrl(ev)`, `buildIcsContent(ev)`, and `icsFilename(ev)` helpers in `Ecosystem.jsx`. Google Calendar link uses `TEMPLATE` action with IANA `ctz` (PT/MT/CT/ET mapped). .ics download builds RFC-5545-compliant VCALENDAR/VEVENT client-side as a `Blob` (text/calendar), triggers `<a download>` click, revokes the URL after 1s. ICS includes UID, DTSTAMP, DTSTART/DTEND (with TZID for timed events, `VALUE=DATE` next-day-exclusive for all-day), SUMMARY, LOCATION, DESCRIPTION (Host + Register URL + brand), URL fields. Comma/semicolon/newline escaping per spec. Filename like `denver-rug-2026.ics`, dedups year suffix when title already contains year. Both links sit side-by-side below "Register now" with identical secondary styling (`CalendarPlus` + `Download` icons). Verified via headless browser download capture on both timed and all-day events.
