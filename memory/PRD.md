...
### Recently shipped (Feb 2026 — Labor Variance marketing page)
- **`/labor-variance`** — self-contained static marketing landing page for a fixed-scope hospital-CFO labor-cost diagnostic. Single new route, no backend, no auth, no CMS. One new file (`frontend/src/pages/LaborVariance.jsx`) with page-scoped design tokens (`--ink`, `--paper`, `--card`, `--line`, `--neutral`, `--favorable`, `--unfavorable`, `--amber`), Archivo/IBM Plex Sans/IBM Plex Mono fonts (added to `public/index.html`), and an inline SVG "Variance Bridge" waterfall — 7 bars (benchmark → 5 additive drivers → actual) with dashed connectors and staggered `scaleY` animations that respect `prefers-reduced-motion`. Sections: sticky header, hero + chart, problem (3 Q cards), methodology ledger (5 drivers with JUSTIFIED / ACTIONABLE pills), how-it-works (4 phase cards + amber panel), deliverables (3 cards), about (2-col with dark intersection panel), contact (full-width ink bg with `mailto:` CTA), footer. Mobile: nav collapses below 720px; chart SVG uses `min-width: 620px` to stay legible via horizontal scroll instead of shrinking labels; every grid collapses to a single column. Global `SiteFooter` suppressed on this route via `GlobalFooter`'s hidden list (page ships its own footer). SEO `<title>` + meta description set in `useEffect` and restored on unmount. Verified: no page-level overflow at 375px or 1920px; button variants all render correctly (a `.lv-btn.lv-btn--primary` specificity boost was needed to beat a global anchor rule inherited from `index.css`).
...
### Recently shipped (Feb 2026 — Pulse daily snapshots)
- **`pulse_daily_snapshots`** collection — one row per UTC day, written at the end of every `run_crawl()` via `_write_daily_snapshot()` in `services/pulse/crawler.py`. Idempotent: upsert on `snapshot_date`, so a same-day manual re-run overwrites rather than duplicates. Write failures are try/except'd around the call site — must never break the crawl. Fields: `snapshot_date` (YYYY-MM-DD, unique), `created_at`, `total_active` (inclusive of vendor per spec), `total_active_ex_vendor`, `by_module {module→count}`, `by_employer_type`, `by_module_and_type [{module, employer_type, count}]`, `by_source`, `new_today`, `marked_inactive_today`, `employers_ok`, `employers_failed`. All aggregations except `total_active` filter `employer_type != "vendor"`. No UI, chart, or endpoint yet — pure historical persistence.
