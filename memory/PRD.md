...
- **[Feb 2026] "Add to calendar" link on EventCard**: exported `buildGoogleCalendarUrl(ev)` builds a Google Calendar TEMPLATE deep-link from title, ISO date, time range (e.g. "4:00–7:00 PM"), timezone abbreviation (PT/MT/CT/ET → IANA), and location. Falls back to all-day event when time is missing/unparseable. Small text link with `CalendarPlus` icon, rendered below "Register now" on both `/ecosystem` and `/ecosystem/events`. Verified via headless run — 5 unit cases pass (timed + DST, all-day, no-time fallback, no-date null, AM/PM inheritance).
